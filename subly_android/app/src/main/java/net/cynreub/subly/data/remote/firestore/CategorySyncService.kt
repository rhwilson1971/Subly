...
@Singleton
class CategorySyncService @Inject constructor(
    ...
    suspend fun upsert(category: Category) {
        ...
        runCatching {
            col(uid).document(category.id.toString())
    ...
    suspend fun fetchAll(uid: String): List<Category> =
        runCatching {
            col(uid).get().await().documents.mapNotNull { it.toCategory() }
